...
class PNGEOFException : public PNGException {
    public:
        PNGEOFException() { error += "Early end of file"; }
};
...
class PNGCRCMismatchException : public PNGException {
    public:
        PNGCRCMismatchException(char const blt[4]);
};
...
#define SEC_READ(STREAM, BUF, LEN) \
do { \
    (STREAM).read(((char*) (BUF)), (LEN)); \
    if ((STREAM).fail()) { \
        throw PNGEOFException(); \
    } \
} while(0);
...
PNGCRCMismatchException::PNGCRCMismatchException(char const *blt) {
    error += "CRC of block ";
    error += blt;
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        uint8_t header[PNG_HDRLEN];
        SEC_READ(png_file, header, 8);
